...
```{r}
write.csv2(df1, "../data/nom.csv", row.names = FALSE)
```
...
df_melt <- df1 %>% 
  pivot_longer(
    cols = everything(),   # Sélectionner toutes les colonnes
    names_to = "categorie", # Colonne des noms des colonnes
    values_to = "nombre"    # Colonne des valeurs des colonnes
  ) %>%
  select(nombre)  # Garder uniquement la colonne 'nombre'
...
```{r}
df3 <- df1 #duplication du jeu de donnée
df3$d <- c(NA, df3$d[-nrow(df1)]) #on modifie pour la colonne d, pb: 1 valeur qui saute
```
