...
Importation du jeu de donnée **'cpf'**
```{r}
cpf <- read.csv("../data/moncompteformation_formations_engagees.csv",sep = ";")
head(cpf)
```
...
cpf <- cpf %>%
  mutate(statut_dossier_modif = case_when(
    ...
    TRUE ~ NA_character_  # gérer les valeurs non spécifiées en leur attribuant NA
  ))

# Réinitialisation de l'index
rownames(cpf) <- NULL

# En tête jeu de donnée
head(cpf)
```

Visualisation de la proportion de personnes ayant terminé la formation par département
```{r}
# Calculer la proportion de personnes ayant terminé la formation par département
proportion_formation_terminee <- cpf %>%
  group_by(departement_lieu_formation) %>%
  summarise(
    proportion_terminee = mean(statut_dossier_modif == "Réalisation totale", na.rm = TRUE)  # Calculer la proportion des personnes ayant terminé
  )

# Réinitialisation de l'index
rownames(cpf) <- NULL

# Afficher les premières lignes pour vérifier les données agrégées
head(proportion_formation_terminee)
```


```{r}
# Créer un graphique à barres avec une taille de texte réduite et les étiquettes des départements pivotées
...
Recherche de doublons : il n'y en a pas ici
```{r}
#sum(duplicated(cpf))
```
...
```{r}
#variable booléen suite au test
verif=cpf$montant_engage==cpf$cout_total

#ajout d'une colonne enregistrant ce booléen
...
```{r}
dep_money <- cpf %>%
  group_by(departement_lieu_formation) %>% #grouper par departement
  summarize(somme_montant_engage = sum(montant_engage, #somme les montants engagés
                                       na.rm = TRUE)) %>% #ignore les valeurs manquantes : NA (on pourrait les garder dans une variable)
  mutate(somme_cumulee = cumsum(somme_montant_engage)) %>% # Ajouter la somme cumulée
  ungroup() %>% # Annuler le groupement pour éviter les conflits
  as.data.frame() #pour qu'il comprenne que c'est un data frame
```
...
train_reg <- cpf %>%
  #select(region_lieu_formation,année.de.validation,prix_moyen)
  filter(année.de.validation == 2022) %>%  # filtrage pour l'année 2022
  group_by(region_lieu_formation) %>%
  summarise(
    nombre_sessions = n(),  # on additionne le nombre de ligne par région
...
```{r}
pop_region=read.csv("../data/fr_population.region.departement.csv",sep=";")
```
...
```{r}
# Filtrer les dossiers avec un statut "Réalisation totale" ou "Réalisation partielle"
real_tot_dep <- cpf %>%
  filter(statut_dossier_modif == "Réalisation totale") %>%
  group_by(departement_lieu_formation) %>%
  summarise(total_dossiers = n(),  # Nombre total de dossiers
            total_realisation = sum(montant_engage, na.rm = TRUE)  # Somme des montants engagés
  ) %>%
  mutate(pourcentage_realisation = total_realisation / total_dossiers * 100)  # Calcul du pourcentage de réalisation
         

# Vérifiez les premiers résultats après filtrage
head(real_tot_dep)
```
...
```{r}
# Fusionner avec pop_formations_reg pour ajouter des informations supplémentaires
